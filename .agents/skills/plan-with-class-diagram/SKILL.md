...
11. `WRITE` 依已載入的類別圖輸出檢查清單與「類別圖提案樣板組」，建立計畫目錄，將完整 Markdown 類別圖提案檔寫入專案根目錄下的 `specs/plan/<NN>-<本次計畫名稱>/class-diagram-proposal.md`；第一個頂層 Section 放置 Mermaid 類別圖，第二個頂層 Section 放置設計說明與實作順序。
12. `CHECK` 逐條驗證類別圖輸出檢查清單，並確認提案檔位於指定路徑、兩個頂層 Section 結構、Mermaid 類別圖、設計說明與實作順序彼此一致且涵蓋需求。

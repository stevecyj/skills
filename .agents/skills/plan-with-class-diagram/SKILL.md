...
3. `THINK` 依已載入的類別設計檢查清單，設計必要的類別、責任、關係、相依性與開發順序。
4. `CHECK` 逐條驗證類別設計檢查清單；未通過時修正設計。
5. `READ` 產生類別圖時，讀取 `rules/類別圖輸出-格式規範.md` 的所有 Rule，建立類別圖輸出檢查清單。
6. `WRITE` 依已載入的類別圖輸出檢查清單產出 Mermaid `classDiagram`、簡短設計說明與實作順序。
7. `CHECK` 逐條驗證類別圖輸出檢查清單，並確認類別圖涵蓋需求且關係一致。

完成條件：類別設計與類別圖輸出檢查清單均已通過，提案可供使用者評估，且程式碼尚未修改。
